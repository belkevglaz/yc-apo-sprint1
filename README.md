...
# Часть 2. Разбивка монолита на микросервисы.

Результирующая схема находится [тут](arch_template_task2.drawio)

По схеме, система выглядит как некий торговый агрегатор.

К сожалению, так как у меня не информации ни о характере нагрузки, описания бизнес-функций и их критичности будем разбивать монолит на сервисы исходя из наименования имеющихся функций, пытаясь сопоставить их возможными бизнес задачами.

Итак, после анализа бизнес-функций монолита получились следующие доменные области:

- Функционал безопасности
  1. Аутентификация пользователя
  2. Профиль пользователя
- Функции администраторов или тех. поддержки
  3. Функционал тех. поддержки
  4. Функционал апелляций
- "Товарный" функционал
  5. Функции работы с товарами
  6. Функции работы с заказами на услугу
  7. Функции работы с предоставлением услуги
  8. Функции работы с аукционом товаров
  9. Поиск
- Отчетность
  10. Функциональность отчетности и статистики
- Уведомления
  11. Функционал уведомлений
- Платежный гейт
  12. Функции проведения платежей

На основе блоков вышеперечисленных блоков функциональности были сформированы соответствующие микросервисы:

1. `Сервис маршрутизации`.
   Представляет собой reverse proxy на базе nginx.  
   Выполняет маршрутизацию запросов от пользователя по соответствующим микросервисам.
   Также на его плечи ложиться некоторый функционал балансировки. Некоторый - потому что полная балансировка будет зависеть от инфраструктуры

2. `Сервис безопастности`.  
   В данном случае, решено было в рамках одного сервиса, на базе Keycloak-а совместить функциональности аутентификации пользователя и его профиля.
   Keycloak является промышленным решением и хорошо справляется с задачами регистрации/аутентификации пользователя, как через базовые механизмы, так и через всевозможные social provider-ы.
   Так же данный инструмент является ПО с открытым кодом, поэтому позволяет делать необходимую кастомизацию при необходимости.
3. `Сервис товаров`  
   Сервис выполняющий функциональность по работе CRUD операциями по товарам, добавляемым пользователем. Объектом данных для него будет сущность `Товар`.
4. `Сервис по работе с заказами`.  
   Сервис для работы с заказами. CRUD операции, обработка заявок на выполнение заказа. Объектом данных будут `Заказ`, `Заявка на выполнение`.
5. `Сервис аукционов`.  
   Аналогично сервису заказов, данный микросервис будет заниматься обработкой аукционов. CRUD операции, обработка заявок на участие в аукционе, обработка ставок и т.п.
6. `Сервис услуг`  
   Аналогично сервису обработки товаров, данный сервис будет заниматься бизнес-функциями связанными с формированием и обслуживанием услуг.
7. `Сервис поиска товаров/услуг`.
   Данный сервис немного специфичен. Предполагаем что, поиск товаров/услуг подразумевает довольно гибкие фильтры поиска, чтобы пользователю было удобно (например полнотекстовый поиск).  
   Поэтому логика работы с поиском была вынесена в отдельный сервис, где данные по товарам/услугам хранятся в полнотекстовом индексе на базе продукта OpenSearch.
   Данный сервис будет по AMQP (об этом чуть позже) получать события о формировании или всевозможных изменения в товаре/услуге и хранить данные для возможностей гибкого поиска.   
   При необходимости данный сервис может обратиться к сервисам товаров/услуг за дополнительными данными
8. `Платежный сервис`  
   Остатки монолита, с функциями проведения оплаты по товарам и заказам.
   В схеме прорисован сценарий входа пользователя, создания заказа на услугу и проведения оплаты за размещение заказа (предполагает что размещение заказа платное)
9. `Сервис отчетности и аудита`  
   Сервис призванный хранить данные по событиям происходящим в системе, а так же предоставлять возможность построения различных отчетов или аудитов.
   Данный сервис подписан на большинство событий (или все) приходящих через AMQP и сохраняющий их в NoSQL базе данных MongoDB.
   MongoDB используем по той причине, что формат payload-а событий довольно сложно будет стандартизировать (разные сервисы пишут в свои объекты данных), а NoSQL БД даст возможность как хранить любые объекты данных, так делать эффективной поиск по ним.
10. `Сервис нотификаций`  
    Сервис для уведомления пользователей о различных событиях в системе. Для уведомлений пользователей будем использовать протокол websocket. Сервис подписан через pub/sub на определенный ряд событий, при получении которых, отправляет клиентам (браузерам) соответствующие данные.
    Также, при необходимости можно реализовать логику доставки уведомлений по другим каналам связи
11. `Сервис техподдержки`  
    Сервис обработки заявок на тех. поддержку.
12. `Сервис апелляций`  
    Сервис обработки спорных вопросов и апелляций.
13. `Брокер сообщений`  
    В качестве инструмента выберем RabbitMQ как один из существующих на рынке брокеров сообщений, умеющих работать по AMQP.  RabbitMQ будет выполнять роль шины, для асинхронного обмена и распространения сообщений между микросервисами. Реализация подписки сервисов основана на возможностях RabbitMQ маршрутизировать сообщения поступающие в exchange в разные очереди используя routing key, тем самым реализуя механизм Publish/Subsribe. Все заинтересованные микросервисы могут подписаться на получение событий, а также сами публиковать события в шину
